...
def build_protected_file_associations(protected_files: List[str]) -> Dict[str, str]:
    return {pattern: "copilot-restricted-file" for pattern in protected_files}
...
def apply_policy_to_claude_settings(
    ...
    permissions["deny"] = replace_managed_claude_deny_rules(
        existing_deny,
...
def apply_policy_to_vscode_settings(
    ...
    updated["chat.tools.edits.autoApprove"] = get_boolean_mapping(
        policy.get("editAutoApprove", {})
    )
...
def run(arguments: Optional[List[str]] = None) -> int:
    parser = ArgumentParser(
        description="Sync .ai-policy.json into agent-specific configuration files."
    )
    parser.add_argument(
        "--import-vscode",
        action="store_true",
        help="Import VS Code approvals into .ai-policy.json first",
    )
    ...
    policy: AiPolicy = read_json_file(
        AI_POLICY,
        {
            "protectedFiles": [],
            "excludedFiles": [],
    ...
    print(
        f"Loaded {len(protected_files)} protected patterns and {len(excluded_files)} excluded patterns"
    )
